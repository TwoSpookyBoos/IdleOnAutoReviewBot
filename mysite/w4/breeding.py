...
def getBreedabilityAdviceGroup():
    ...
    max_heart_level = len(breedabilityDaysList)
    total_pet = len(sorted_breedability)
    total_by_heart = {
        f"breedability-heart-{index}": 0 for index in range(0, max_heart_level)
    }
    achievement_7s = 0
    ...
    for heart_level, heart in enumerate(total_by_heart.keys()):
        if total_by_heart[heart] > 0:
            # Found lowest breedability heart
            break
    if heart_level > 6:
        # On 7 level or higher
        if total_by_heart[heart] == total_pet and heart_level < max_heart_level:
            # All pet at lowest level, move target one step ahead (if have that step)
    ...
    target = breedabilityDaysList[heart_level]
    for pet in sorted_breedability:
    ...
    b_advices.insert(0, Advice(
        label=f"Total Heart {number_to_roman(heart_level)}+ pets: {total_by_heart[heart]}/{total_pet}",
        picture_class=heart,
        progression=sum(
            [
