...
def getLabAdviceSection() -> AdviceSection:
    ...
    #TODO debug call :
    calculate_players_line_width(1, 1, 1, 1)
...
def calculate_players_line_width(lab, breeding, gem_store, bubo_boost):
    meal_bonus = 0  # 1.59 Change - 16th Jewel no longer impacts meal jewels

    if True: #if lab.players_in_tubes:
        px_meal_bonus = sum(
            session_data.account.meals["Pancakes"]["Level"] * session_data.account.meals["Pancakes"]["BaseValue"],
            session_data.account.meals["Wild Boar"]["Level"] * session_data.account.meals["Wild Boar"]["BaseValue"]
        )
        line_pct_meal_bonus = session_data.account.meals["Eel"]["Level"] * session_data.account.meals["Eel"]["BaseValue"]
        
        ########################################################### Stopped converting here
        # TODO Parse per-player equipped cards
        passive_card_bonus = 0 #sum(card.getBonus() for card in (cards or []) if "Line Width" in card.data.effect)
