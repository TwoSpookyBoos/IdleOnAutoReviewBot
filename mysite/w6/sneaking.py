...
def setSneakingProgressionTier():
    ...
    for gemstoneName, gemstoneLevel in session_data.account.sneaking["Gemstones"].items():
        sneaking_AdviceDict["Gemstones"].append(Advice(
            label=f"{gemstoneName} ({session_data.account.sneaking.get('Gemstones', {}).get(gemstoneName, {}).get('Level', 0)}: {session_data.account.sneaking.get('Gemstones', {}).get(gemstoneName, {}).get('Value', 0):.2f})",
