...
def computeFinalBalladMultiplier(final_ballad_acquired, num_trebel_notes):
    if not final_ballad_acquired or num_trebel_notes == 0:
        return 1.0
    return 1 + 0.04 * math.floor(math.log10(num_trebel_notes) + 1)
...
def getSnailInformationGroup():
    ...
    if sodium_safety_level + 5 <= snail_rank:
        ...
        if final_ballad_acquired:
            ...
            snail_AdviceDict["General"].append(Advice(
                label=f"Final Ballad of the Snail acquired! {num_trebel_digits} trebel note digits for a bonus of {final_ballad_bonus:0.2f}x",
                picture_class="gaming", # Placeholder
            ))
        else:
            snail_AdviceDict["General"].append(Advice(
