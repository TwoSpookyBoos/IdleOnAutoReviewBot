...
summoning_battle_per_color_dict: dict[str, list[dict]] = {}
for color, enemy_list in summoning_enemy_per_color_dict.items():
    summoning_battle_per_color_dict[color] = []
    for enemy in enemy_list:
        match_index = SummonEnemies[0].index(enemy)
        summoning_battle_per_color_dict[color].append(
            {
                "EnemyID": enemy,
                "OpponentName": SummonEnemies[4][match_index].replace("_", " "),
                "RewardIndex": parse_number(SummonEnemies[5][match_index]),
                "RewardQTY": parse_number(SummonEnemies[7][match_index]),
            }
        )

# To verify we aren't missing any summoning enemies, we check for any new entries
# that aren't in the unimplemented matches list
all_assigned_enemy = [
    enemy
...
summoning_unassigned_enemy_names = [
    enemy_name
    for enemy_name in SummonEnemies[0]
    if enemy_name not in all_assigned_enemy
]
if len(summoning_unassigned_enemy_names) > 0:
    logger.error(
        f"Unplaced summoning matches found: {summoning_unassigned_enemy_names}."
        " Add them to the proper color in summoning_matches_per_color_dict."
    )

# summoning_endless_challenge_types = SummonEnemies[12]
summoning_endless_challenge_types = [
    "Slow Motion (60% speed)",
    "Fast Forward (170% speed)",
    "Glass Cannon (1 HP)",
    "Zerg Surprise",
    "Extra Time (2x HP)",
    "Fair Play (No lane stacking)",
    "Invincibility",
    "Offsides Rule",
    "Triple Overtime (10x HP)",
    "Truce (No Mods)",
    "Uno Draw 7",
]
if len(SummonEnemies[12]) > len(summoning_endless_challenge_types):
    logger.warning("Endless Summoning Challenge Types out of sync.")

summoning_endless_battle_dict = {
    index: {
        "RewardIndex": parse_number(SummonEnemies[9][index]),
        "RewardQTY": parse_number(SummonEnemies[10][index]),
        "Challenge": summoning_endless_challenge_types[int(SummonEnemies[11][index])],
    }
    for index in range(0, 40)
}

summoning_stone_boss_base_hp = [parse_number(hp) for hp in SummonEnemies[14]]
summoning_stone_boss_base_damage = [
    parse_number(damage) for damage in SummonEnemies[13]
]
