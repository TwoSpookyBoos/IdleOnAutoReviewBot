...
def get_tesseract_currencies_advice_group(tesseract) -> AdviceGroup:
    ...
    ## IF statement for Arcane Rock collection advice
    ...
    for arcane_cultist in session_data.account.acs:
        ...
        if (
            arcane_cultist.current_preset_talents.get(str(tesseract_talent_index), 0)
            > tesseract_preset_level
        ):
            arcane_cultist_index = arcane_cultist.character_index
            tesseract_preset_level = arcane_cultist.current_preset_talents.get(
                str(tesseract_talent_index), 0
            )
        if (
            arcane_cultist.secondary_preset_talents.get(str(tesseract_talent_index), 0)
            > tesseract_preset_level
        ):
            tesseract_preset_level = arcane_cultist.secondary_preset_talents.get(
                str(tesseract_talent_index), 0
            )
    ...
    missing_bundle_data_txt = (
        "<br>Note: Could be inaccurate. Bundle data not found!"
        if not session_data.account.gemshop["Bundle Data Present"]
        else ""
    )
    ...
    upgrades_advice_dict["Upgrades"] += [
        Advice(
            label=(
                ...
                if not upgrade_details["Unlocked"]
                else f"{upgrade_name}: {upgrade_details['Description']}"
            ),
...
def get_tesseract_advice_section() -> AdviceSection:
    ...
    tesseract_advice_group_dict["Tiers"], overall_section_tier, max_tier, true_max = (
        get_progression_tiers_advice_group()
    )
    tesseract_advice_group_dict["Currencies"] = get_tesseract_currencies_advice_group(
        tesseract
    )
    tesseract_advice_group_dict["Upgrades"] = get_tesseract_upgrades_advice_group(
        tesseract
    )
