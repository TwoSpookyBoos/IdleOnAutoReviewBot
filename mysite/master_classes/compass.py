...
def getCompassCurrenciesAdviceGroup(compass):
    ...
    currency_advices["Currencies"].append(
        Advice(
            ...
            progression=compass["Upgrades"]["Top of the Mornin'"]["Total Value"]
            + compass["Upgrades"]["Abomination Slayer XII"]["Total Value"]
            - compass["Top of the Mornin'"],
            goal=compass["Upgrades"]["Top of the Mornin'"]["Total Value"]
            + compass["Upgrades"]["Abomination Slayer XII"]["Total Value"],
            informational=True,
        )
    )
    ...
    if compass["Aethermoons Enabled"]:
        ...
        currency_advices["Currencies"].append(
            Advice(
                ...
                progression=min(
                    1000, session_data.account.all_assets.get("Quest100").amount
                ),
                goal=1000,
            )
        )

    currency_advices["Currencies"].extend(
        [
            Advice(
    ...
    currency_advices[mgc_label] = [
        Advice(
            ...
            picture_class=session_data.account.sneaking["PristineCharms"][
                "Twinkle Taffy"
            ]["Image"],
            progression=int(
                session_data.account.sneaking["PristineCharms"]["Twinkle Taffy"][
                    "Obtained"
                ]
            ),
            goal=1,
    ...
    currency_advices[mgd_label] = [
        Advice(
            ...
            progression=int(
                session_data.account.all_assets.get("EquipmentHats118").amount > 0
            ),
            goal=1,
            resource="gem",
        ),
        Advice(
            ...
            progression=int(
                session_data.account.all_assets.get("EquipmentBowsTempest0").amount > 0
            ),
            goal=1,
        ...
        Advice(
            ...
            progression=min(
                session_data.account.all_assets.get("EquipmentRingsTempest6").amount, 2
            ),
            goal=2,
    ...
    bonus_talent_levels = (
        session_data.account.all_characters[ww_index].total_bonus_talent_levels
        if ww_index is not None
        else 0
    )
    ...
    for bonus_name in [
        ...
        "De Dust IV",
        "De Dust V",
        "Abomination Slayer IX",
        "Abomination Slayer XXX",
        "Abomination Slayer XXXIV",
    ]:
        currency_advices[mgf_label].append(
            Advice(
...
def getCompassAbominationsAdviceGroup(compass):
    ...
    abom_ag = AdviceGroup(
        tier="", pre_string="Abominations", advices=abom_advices, informational=True
    )
...
def getCompassMedallionsAdviceGroup(compass):
    ...
    medallion_ag = AdviceGroup(
        tier="", pre_string="Medallions", advices=medallion_advice, informational=True
    )
...
def getCompassUpgradesAdviceGroups(compass):
    ...
    for path_name in compass_path_ordering:
        ...
        for compass_upgrade_index in compass_path_ordering[path_name]:
            clean_name = (
                compass_upgrades_list[compass_upgrade_index][0]
            ...
            if path_name == "Abomination":
                if (
                    "Titan doesnt exist" not in upgrade_details["Description"]
                ):  # Filter out placeholders for future Titans/Abominations
                    abom = compass["Abominations"].get(
                        upgrade_details["Abomination Name"], {"World": "?"}
                    )
    ...
    upgrades_AdviceDict["Default Path Upgrades"].insert(
        0,
        Advice(
    ...
    upgrades_AdviceDict["Abomination Path Upgrades"].insert(
        0,
        Advice(
...
def getCompassAdviceSection() -> AdviceSection:
    ...
    compass_AdviceGroupDict["Tiers"], overall_SectionTier, max_tier, true_max = (
