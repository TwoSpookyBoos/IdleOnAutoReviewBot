...
class FamilyBonus:
    def __init__(self, class_name: str, info: dict, level: int):
        ...
        self.value: float = self.value_at_level(level)
...
class FamilyBonuses(dict[str, FamilyBonus]):
    def __init__(self, characters: list[Character]):
        super().__init__()
        levels = dict.fromkeys(family_bonuses_dict, 0)
        for character in characters:
            for class_name in character.all_classes:
                if class_name in levels:
                    levels[class_name] = max(levels[class_name], character.combat_level)
        for class_name, info in family_bonuses_dict.items():
            self[class_name] = FamilyBonus(class_name, info, levels[class_name])
