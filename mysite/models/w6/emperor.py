...
class EmperorBonus:
    def __init__(self, index: int, wins: int):
        ...
        self._template, (self._bonus_type, display_name) = (
            emperor_bonus_description[index]
        )
    ...
    def get_bonus_advice(self, link_to_section: bool = True):
        label = ""
        if link_to_section:
            label += "{{Emperor Showdowns|#emperor}} - "
...
class Emperor(dict[str, EmperorBonus]):
    ...
    def _calculate_emperor_health(self, showdowns_index: int) -> str:
        # _customBlock_Thingies -> if ("Boss6HP" == d)
        # Last updated in v2.48 Giftmas Event (December 8, 2025).
        hp = 135e12 * safer_math_pow(1.54, showdowns_index)
