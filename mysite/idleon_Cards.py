...
def getCardSetReview(inputJSON):
    #0star always requires 1 card
    #1star is set per enemy
    #2star = 3x additional what 1star took, for a total of 4x
    #3star = 5x additional what 1star took, for a total of 4+5 = 9x
    #4star = 16x additional what 1star took, for a total of 9+16 = 25x
    #5star = 459 additional what 1star took, for a total of 25+459 = 484x
    cardRequirementsDict = {
        ...
        "Hard Resources": {
            ...
            "Godshard": ["Godshard Ore", 400, 1600, 3600, 10000, 193600],
            ...
            "Tree12": ["Bamboo", 15, 60, 135, 375, 7260],
            "Tree13": ["Effervescent Logs", 15, 60, 135, 375, 7260],
            ...
            "Bug12": ["Ladybug", 15, 45, 75, 240, 6885],
            "Bug13": ["Firefly", 15, 45, 75, 240, 6885],
            ...
            "SoulCard7": ["Breezy Soul", 7, 21, 35, 112, 3213],
        ...
        "Spirited Valley": {
            "w6a1": ["Sprout Spirit", 50, 150, 250, 800, 22950],
            "w6a2": ["Ricecake", 60, 180, 300, 960, 27540],
            "w6a3": ["River Spirit", 75, 225, 375, 1200, 34425],
            "w6a4": ["Baby Troll", 85, 255, 425, 1360, 45900],
            "w6a5": ["Woodlin Spirit", 100, 300, 500, 1600, 45900],
            "w6b1": ["Bamboo Spirit", 150, 450, 750, 2400, 68850],
            "w6b2": ["Lantern Spirit", 170, 510, 850, 2720, 78030],
            "w6b3": ["Mama Troll", 200, 600, 1000, 3200, 91800],
            "w6b4": ["Leek Spirit", 250, 750, 1250, 4000, 114750],
            "w6c1": ["Ceramic Spirit", 400, 1200, 2000, 6400, 183600],
            "w6c2": ["Skydoggie Spirit", 500, 1500, 2500, 8000, 229500],
            "w6d1": ["Royal Egg", 900, 2700, 4500, 14400, 413100],
            "w6d2": ["Minichief Spirit", 1300, 3900, 6500, 20800, 596700],
            "w6d3": ["Samurai Guardian", 2500, 7500, 12500, 40000, 1147500],
            "Crystal5": ["Crystal Brighton", 5000, 15000, 25000, 80000, 2295000]
        },
        "Dungeons": {
        ...
        "Bosses n Nightmares": {
            ...
            "Boss6A": ["Emperor", 6, 18, 30, 96, 2754],
            "Boss6B": ["Chaotic Emperor", 9, 27, 45, 144, 4131],
            "Boss6C": ["Sovereign Emperor", 13, 39, 65, 208, 5967],
        },
        "Events": {
    ...
    for card in rawCardsData:
        ...
        for cardSet in cardRequirementsDict:
            ...
            if card in cardRequirementsDict[cardSet]:
                ...
                if rawCardsData[card] >= cardRequirementsDict[cardSet][card][5] + cardRequirementsDict[cardSet][card][
                    4] + cardRequirementsDict[cardSet][card][3] + cardRequirementsDict[cardSet][card][2] + \
                        cardRequirementsDict[cardSet][card][1]:
    ...
    for cardSet in cardSetBonusRequirements:
        ...
        playerCardSetBonuses[cardSet] = str(playerCardSetTotalsDict[cardSet]) + "/" + str(
            nextCardSetBonusTierRequirement) + " for " + playerNextBonusTierName
    ...
    for cardSet in playerCardsToNextLevelDict:
        for card in playerCardsToNextLevelDict[cardSet]:
            if playerCardsToNextLevelDict[cardSet][card]["CardsToNextLevel"] in closestCardsPerSet[cardSet]:
                closestCardsPerSet[cardSet][playerCardsToNextLevelDict[cardSet][card]["CardsToNextLevel"]].append(
                    playerCardsToNextLevelDict[cardSet][card]["CardName"] + " (" +
                    playerCardsToNextLevelDict[cardSet][card]["NextLevelName"] + ")")
            else:
                closestCardsPerSet[cardSet][playerCardsToNextLevelDict[cardSet][card]["CardsToNextLevel"]] = [
                    playerCardsToNextLevelDict[cardSet][card]["CardName"] + " (" +
                    playerCardsToNextLevelDict[cardSet][card]["NextLevelName"] + ")"]
