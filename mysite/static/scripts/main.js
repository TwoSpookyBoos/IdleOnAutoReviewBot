...
function calcProgressBars(parent = document) {
    console.log("calculating progress bars")
    parent.querySelectorAll(".progress-box").forEach(progressBox => {
        ...
        if (checkbox.value === "off" || !show) {
            ...
            console.log("unhiding progress bar")
...
const hideProgressBoxes = (mode = true, parent = document) => parent
    .querySelectorAll('.progress-box')
    .forEach(box => mode ? box.classList.add("hidden") : box.classList.remove("hidden"))

let resizeTimer;
window.addEventListener('resize', () => {
    if (!resizeTimer) hideProgressBoxes(true)
